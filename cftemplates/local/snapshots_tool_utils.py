...
_DESTINATION_REGION = 'us-east-1'

_KMS_KEY_DEST_REGION = ''

_KMS_KEY_SOURCE_REGION = 'arn:aws:kms:us-east-1:349213950191:key/e5a5c7d5-178f-4aa2-9c28-1fa102dfce2c'
...
def copy_local(snapshot_identifier, snapshot_object):
    ...
    if snapshot_object['StorageEncrypted']:
        print('Copying encrypted snapshot %s locally' %
                    snapshot_identifier)
        ...
        print('Copying snapshot %s locally' % snapshot_identifier)
...
def copy_remote(snapshot_identifier, snapshot_object):
    ...
    if snapshot_object['StorageEncrypted']:
        print('Copying encrypted snapshot %s to remote region %s' %
        ...
        print('Copying snapshot %s to remote region %s' %
                    (snapshot_object['Arn'], _DESTINATION_REGION))
