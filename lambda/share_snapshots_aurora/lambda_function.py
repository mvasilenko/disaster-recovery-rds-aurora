...
SNAPSHOTS_PATTERN = os.getenv('SNAPSHOTS_PATTERN', '^stool.*$')
BACKUP_NAME_PREFIX = os.getenv('BACKUP_NAME_PREFIX', 'stool')
...
def lambda_handler(event, context):
    ...
    for snapshot_identifier,snapshot_object in filtered.items():
        ...
        if snapshot_object['Status'].lower() == 'available' and search_tag_share(response_tags):
            ...
            logger.info("Check for KMS encryption on db: {}".format(snapshot_identifier))
            kms = False
            if snapshot_info['DBClusterSnapshots'][0]['StorageEncrypted'] == True:
                try:
                    ...
                    logger.info('Exception get_kms_type {}: {}'.format(snapshot_identifier, e))
                    logger.info('Its a copied snapshot with a key from DEST account')
            else:
                logger.info("Snapshot: {} is not encrypted".format(snapshot_identifier))

            logger.info('Checking Snapshot: {}'.format(snapshot_identifier))
            if kms is True and BACKUP_KMS != '':
                try:
                    logger.info('Running copy: {}'.format(snapshot_identifier))
            ...
            try:
                ...
                logger.info('Sharing: {}'.format(snapshot_to_copy))
                ...
                logger.error('Exception sharing {}: {}'.format(snapshot_to_copy, e))
                pending_snapshots += 1
